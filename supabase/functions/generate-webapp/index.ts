...
CRITICAL: Always ensure all file contents in the response are valid strings. Never return objects or arrays as file content - convert them to proper string format (JSON.stringify for JSON files, proper code syntax for code files).`;
}
...
function parseAIResponse(content: string) {
  ...
  if (parsedResponse.files) {
    Object.keys(parsedResponse.files).forEach(fileName => {
      const content = parsedResponse.files[fileName];
      if (typeof content !== 'string') {
        if (content && typeof content === 'object') {
          parsedResponse.files[fileName] = JSON.stringify(content, null, 2);
        } else {
          parsedResponse.files[fileName] = String(content || '');
        }
      }
    });
  }
