...
const LoadingAnimation = ({
  ...
  if (variant === "dots") {
    return <div className={cn("flex items-center justify-center gap-1", sizeClasses[size], className)}>
        {[0, 1, 2].map(i => <div key={i} className={cn("w-2 h-2 rounded-full", colors.primary, "animate-pulse")} style={{
  ...
  if (variant === "pulse") {
    return <div className={cn("relative", sizeClasses[size], className)}>
        <div className={cn("absolute inset-0 rounded-full", colors.primary, "animate-ping opacity-75")} />
        <div className={cn("relative w-full h-full rounded-full", colors.primary, "animate-pulse")} />
      </div>;
  }
  if (variant === "orbit") {
    return;
  ...
  if (variant === "neural") {
    return <div className={cn("relative flex items-center justify-center", sizeClasses[size], className)}>
        ...
        <div className="relative w-full h-full">
          {/* Center node */}
          <div className={cn("absolute top-1/2 left-1/2 w-3 h-3 -translate-x-1/2 -translate-y-1/2 rounded-full", colors.primary, "animate-pulse shadow-lg", colors.glow)} />
          ...
          return <div key={i} className={cn("absolute w-2 h-2 rounded-full", colors.secondary, "animate-pulse")} style={{
            left: `calc(50% + ${x}px)`,
            top: `calc(50% + ${y}px)`,
          ...
          return <div key={`line-${i}`} className={cn("absolute w-px opacity-30", colors.accent, "animate-pulse")} style={{
            left: '50%',
            top: '50%',
            height: `${length}px`,
          ...
          {/* Outer ring */}
          <div className={cn("absolute inset-0 rounded-full border-2 border-dashed opacity-20", `border-${color}-400`, "animate-spin")} style={{
          animationDuration: "8s"
        }} />
          
          {/* Inner ring */}
          <div className={cn("absolute inset-2 rounded-full border border-solid opacity-30", `border-${color}-300`, "animate-spin")} style={{
        ...
        {/* Pulsing glow effect */}
        <div className={cn("absolute inset-0 rounded-full opacity-20 animate-ping", colors.primary)} style={{
        animationDuration: "3s"
      }} />
      </div>;
  }
  ...
  return <div className={cn("relative flex items-center justify-center", sizeClasses[size], className)}>
      <div className="relative w-full h-full">
        {/* Central prism shape */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 prism-shape" style={{
        ...
        return <div key={`ray-${i}`} className="absolute prism-ray" style={{
          left: '50%',
          top: '50%',
          width: '2px',
          height: `${length}px`,
        ...
        const x = Math.cos(angle) * radius;
        const y = Math.sin(angle) * radius;
        return <div key={`particle-${i}`} className="absolute w-1 h-1 rounded-full prism-particle" style={{
          left: `calc(50% + ${x}px)`,
          top: `calc(50% + ${y}px)`,
          transform: 'translate(-50%, -50%)',
        ...
        {/* Rotating light beam */}
        <div className="absolute inset-0 prism-beam" style={{
        ...
        {/* Outer glow ring */}
        <div className="absolute inset-0 rounded-full prism-glow" style={{
