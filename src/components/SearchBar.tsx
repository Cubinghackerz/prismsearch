...
const SearchBar = ({
  ...
  const getRandomPosition = () => {
    return {
      x: Math.random() * 100,
      // Random position along width (0-100%)
      y: Math.random() < 0.5 ? 0 : 100,
      // Either top or bottom of the search bar
      side: Math.random() < 0.5 ? 'left' : 'right',
      // Which side spark comes from
      delay: Math.random() * 0.5,
      // Random delay for staggered effect
      duration: 0.8 + Math.random() * 1.2,
      // Random duration between 0.8s and 2s
      size: 0.5 + Math.random() * 0.8 // Random size factor between 0.5 and 1.3
    };
  };
  return <div className={`search-bar-container w-full max-w-3xl mx-auto transition-all duration-500 ${expanded ? 'search-bar-expanded' : ''}`}>
      <div className="relative">
        <div ref={searchBarRef} className={`
          ...
          <div className="relative flex items-center h-full w-full rounded-2xl group overflow-hidden">
            ...
            <input ref={inputRef} value={query} onChange={e => handleInputChange(e.target.value)} onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => {
            handleKeyDown(e);
          ...
          }} onBlur={() => setIsFocused(false)} className="peer h-full w-full outline-none text-lg text-white/90 px-2 bg-transparent 
                placeholder:text-orange-200/30 
                transition-all duration-300 
                placeholder:transition-opacity placeholder:duration-300
                focus:placeholder:opacity-50
                focus:text-white" type="text" placeholder="Search across the web..." disabled={isSearching} aria-label="Search input" aria-autocomplete="list" aria-controls="search-suggestions" aria-expanded={isOpen} />
            
            {/* Search options button - Only show bookmarks option */}
            {!isSearching}
            
            {query && !isSearching && <button onClick={handleClear} className="absolute right-28 h-8 w-8 flex items-center justify-center text-orange-300/70 
            ...
            <button onClick={handleSearch} disabled={isSearching || !query.trim()} className={`
                absolute right-2 h-12 w-24 rounded-xl text-white font-medium
                transition-all duration-300 
                ${query.trim() && !isSearching ? 'bg-gradient-to-r from-orange-500 to-orange-700 hover:from-orange-600 hover:to-orange-800 cursor-pointer shadow-lg shadow-orange-900/30 hover:shadow-orange-800/40 hover:scale-105 active:scale-95 ember-glow' : 'bg-gray-700/30 cursor-not-allowed opacity-50'}
            ...
            {isFocused && !isSearching && <>
              ...
              return <motion.div key={`spark-${i}-${sparkTrigger}`} className="absolute pointer-events-none fiery-spark" style={{
                ...
                y: sparkProps.y === 0 ? ['0px', '-30px', '-60px'] : ['0px', '30px', '60px'],
                x: sparkProps.side === 'left' ? ['0px', '20px', '40px'] : ['0px', '-20px', '-40px']
              }} transition={{
        ...
        <div ref={dropdownRef} className="relative w-full">
          <AnimatePresence>
            ...
            <AutocompleteDropdown suggestions={suggestions} isOpen={isOpen && isFocused} highlightedIndex={highlightedIndex} onSelectSuggestion={handleSuggestionClick} inputValue={query} />
          </AnimatePresence>
        </div>
