    ...
    duration: 0.5
      ...
      <div className="flex flex-wrap items-center justify-between mb-4 gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map(category => <motion.button key={category} onClick={() => setSelectedCategory(category as SearchCategory | 'All')} className={`px-3 py-1 text-xs rounded-full transition-all border ${selectedCategory === category ? 'bg-orange-600 text-white border-orange-500/50' : 'bg-orange-500/20 text-orange-200 hover:bg-orange-500/30 border-orange-500/10'}`} whileHover={{
          scale: 1.05
        }} whileTap={{
          scale: 0.95
        }}>
      ...
      <div className="space-y-4">
        {trendingSearches.length > 0 && <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {trendingSearches.map((search, index) => <TrendingSearchItem key={index} search={search} onClick={() => onSelectSearch(search.query)} index={index} />)}
            </div>
          </div>}
        ...
        {regularSearches.length > 0 && <div>
            ...
            <div className="flex flex-wrap gap-2">
              {regularSearches.map((search, index) => <RegularSearchItem key={index} search={search} onClick={() => onSelectSearch(search.query)} index={index} />)}
            </div>
          </div>}
      </div>
    </motion.div>;
...
const TrendingSearchItem = ({
  ...
  return <motion.div initial={{
    ...
    scale: 0.98
  }} onClick={onClick} className="p-3 rounded-lg cursor-pointer transition-all bg-gradient-to-br from-orange-600/80 to-orange-700/80 border border-orange-500/30 shadow-md hover:shadow-orange-500/20">
      ...
      <div className="flex items-center mt-2 text-xs text-orange-200">
        ...
        {search.category && <span className="ml-auto bg-orange-500/20 px-2 py-0.5 rounded text-orange-200 border border-orange-500/10">
            {search.category}
          </span>}
      </div>
    </motion.div>;
...
const RegularSearchItem = ({
  ...
  return <motion.div initial={{
    ...
    scale: 0.95
  }} onClick={onClick} className="px-3 py-2 rounded-lg cursor-pointer transition-all flex items-center space-x-2 bg-orange-500/10 border border-orange-500/20 hover:bg-orange-500/20 hover:border-orange-500/30">
      <History className="h-3 w-3 text-orange-300" />
      <span className="text-white text-sm font-inter">{search.query}</span>
      {search.category && <span className="text-xs bg-orange-500/20 px-2 py-0.5 rounded text-orange-200 border border-orange-500/10">
          {search.category}
        </span>}
    </motion.div>;
};
