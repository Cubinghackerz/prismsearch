...
const PopularSearches = ({ onSelectSearch, currentQuery }: PopularSearchesProps) => {
  ...
  return (
    <div className="w-full bg-[#1A1F2C]/95 backdrop-blur-md rounded-xl border border-purple-500/30 p-5 shadow-lg mb-6">
...
const TrendingSearchItem = ({ search, onClick }: SearchItemProps) => {
  return (
    <motion.div
      whileHover={{ scale: 1.03 }}
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className="bg-purple-500/30 hover:bg-purple-500/40 p-3 rounded-lg cursor-pointer transition-all"
    >
      ...
      <div className="flex items-center mt-2 text-xs text-purple-200">
        ...
        {search.category && (
          <span className="ml-auto bg-purple-800/40 px-2 py-0.5 rounded text-purple-200">
            {search.category}
          </span>
        )}
      </div>
...
const RegularSearchItem = ({ search, onClick }: SearchItemProps) => {
  return (
    <motion.div
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      onClick={onClick}
      className="bg-purple-500/20 hover:bg-purple-500/30 px-3 py-2 rounded-lg cursor-pointer transition-all flex items-center space-x-2"
    >
      <History className="h-3 w-3 text-purple-300" />
      <span className="text-white text-sm">{search.query}</span>
      {search.category && (
        <span className="text-xs bg-purple-800/40 px-2 py-0.5 rounded text-purple-200">
          {search.category}
        </span>
      )}
    </motion.div>
