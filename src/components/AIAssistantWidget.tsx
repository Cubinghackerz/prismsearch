...
const AIAssistantWidget = () => {
  ...
  if (!isOpen) {
    return <div className="fixed bottom-6 right-6 z-50">
        <Button onClick={() => setIsOpen(true)} className="w-14 h-14 rounded-full bg-gradient-to-r from-prism-primary to-prism-accent hover:from-prism-primary-dark hover:to-prism-accent-dark text-white shadow-2xl hover:shadow-prism-primary/25 transition-all duration-300 group">
      ...
      <Card className={`w-96 shadow-2xl border-prism-border/50 bg-prism-surface/95 backdrop-blur-md transition-all duration-300 ${isMinimized ? 'h-16' : 'h-[500px]'}`}>
        <CardHeader className="flex flex-row items-center space-y-0 pb-2 px-4 py-3 border-b border-prism-border/30">
          <div className="flex items-center space-x-2 flex-1">
            ...
            <CardTitle className="text-sm font-semibold text-prism-text">AI Assistant (Beta)</CardTitle>
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
          </div>
          <div className="flex items-center space-x-1">
            <Button variant="ghost" size="sm" onClick={() => setIsMinimized(!isMinimized)} className="w-8 h-8 p-0 hover:bg-prism-surface/50">
              {isMinimized ? <Maximize2 className="w-4 h-4" /> : <Minimize2 className="w-4 h-4" />}
            </Button>
            <Button variant="ghost" size="sm" onClick={() => setIsOpen(false)} className="w-8 h-8 p-0 hover:bg-prism-surface/50">
              <X className="w-4 h-4" />
            </Button>
          </div>
        </CardHeader>

        {!isMinimized && <CardContent className="p-0 flex flex-col h-[calc(500px-64px)]">
            <ScrollArea className="flex-1 px-4 py-2">
              <div className="space-y-4">
                {messages.map(message => <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-lg text-sm ${message.isUser ? 'bg-gradient-to-r from-prism-primary to-prism-accent text-white' : 'bg-prism-surface border border-prism-border/30 text-prism-text'}`}>
                      <p className="whitespace-pre-wrap">{message.content}</p>
                      <p className={`text-xs mt-1 ${message.isUser ? 'text-white/70' : 'text-prism-text-muted'}`}>
                ...
                {isTyping && <div className="flex justify-start">
                    <div className="bg-prism-surface border border-prism-border/30 p-3 rounded-lg">
                      <div className="flex space-x-1">
                        ...
                        <div className="w-2 h-2 bg-prism-primary rounded-full animate-bounce" style={{
                    animationDelay: '0.1s'
                  }} />
                        <div className="w-2 h-2 bg-prism-primary rounded-full animate-bounce" style={{
                    animationDelay: '0.2s'
            ...
            <div className="p-4 border-t border-prism-border/30">
              <div className="flex space-x-2">
                <Input value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="Ask me anything..." className="flex-1 border-prism-border/30 bg-prism-surface/50 text-prism-text placeholder:text-prism-text-muted" />
                <Button onClick={handleSendMessage} disabled={!inputValue.trim() || isTyping} className="bg-gradient-to-r from-prism-primary to-prism-accent hover:from-prism-primary-dark hover:to-prism-accent-dark text-white">
