  ...
  return <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden">
          <div className={`h-2 rounded-full transition-all duration-500 ${getStrengthBarColor(assessment.score)}`} style={{
      ...
      {assessment.feedback.length > 0 && <div className="space-y-2">
          <span className="text-sm text-slate-200 font-medium">Security Recommendations</span>
          <ul className="text-sm text-slate-400 space-y-1">
            {assessment.feedback.map((item, index) => <li key={index} className="flex items-start space-x-2">
